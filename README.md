...
- Replicate the design and results of the PANDA framework-architectural modifications of nnU-Net for multi-task learning (joint segmentation and classification).  
- Dataset (private): A small set of 3D CT scans of pancreatic cancer patients (252 scans).  
...
We used a training dataset of 252 3D CT scans of the pancreas region. Each scan is labeled with one of 3 subtypes, and has a corresponding egmentation mask.  
...
> We did not apply additional custom augmentations or transformations; we used nnU-Net’s built-in augmentations (via `batchgenerators`).
