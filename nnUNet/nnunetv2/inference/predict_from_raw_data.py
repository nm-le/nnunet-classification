...
class nnUNetPredictor(object):
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if ofile is not None:
                    ...
                    cls_file = ofile + "_class.txt"
                    predicted_class = int(torch.argmax(cls_prediction, dim = 1).item()) \
                        if cls_prediction.dim() > 1 else int(torch.argmax(cls_prediction).item())
                    
                    with open('/data/ngcminhle/uhn_predictions.csv', 'a') as f:
                        # f.write(f"Classification logits: {str(cls_prediction)}\n")
                        file_name = ofile.split("/")[-1]
                        f.write(f"{file_name},{str(predicted_class)}\n")
                    r.append(
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for params in self.list_of_parameters:
            ...
            seg_pred, cls_pred = self.predict_sliding_window_return_logits(data)
            seg_pred = seg_pred.to('cpu')
            cls_pred = cls_pred.to('cpu')

            if seg_ensemble is None:
                seg_ensemble = seg_pred
                cls_ensemble = cls_pred
            else:
                seg_ensemble += seg_pred
                cls_ensemble += cls_pred
        ...
        seg_ensemble /= len(self.list_of_parameters)
        cls_ensemble /= len(self.list_of_parameters)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        seg_prediction, class_prediction = self.network(x) 
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                seg_prediction += torch.flip(self.network(torch.flip(x, axes))[0], axes)
                # print(f"Class prediction flipped: {str(self.network(torch.flip(x, axes))[1])}")
                print(f"Class prediction unflipped: {class_prediction}")
                class_prediction += self.network(torch.flip(x, axes))[1]
            seg_prediction /= (len(axes_combinations) + 1)
            class_prediction /= (len(axes_combinations) + 1)

        return seg_prediction, class_prediction
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        seg_logits = cls_logits = n_predictions = seg_pred = cls_pred = gaussian = workon = None
        num_forward_pass = 0
        ...
        try:
            ...
            seg_logits = torch.zeros((self.label_manager.num_segmentation_heads, *data.shape[1:]),
                                           dtype=torch.half,
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                seg_pred, cls_pred = self._internal_maybe_mirror_and_predict(workon)
                seg_pred = seg_pred[0].to(results_device)

                if cls_logits is None:
                    cls_logits = cls_pred.clone()
                else:
                    cls_logits += cls_pred
                
                if self.use_gaussian:
                    seg_pred *= gaussian
                seg_logits[sl] += seg_pred
                n_predictions[sl[1:]] += gaussian

                num_forward_pass += 1

            seg_logits /= n_predictions
            cls_logits /= num_forward_pass
            # check for infs
            if torch.any(torch.isinf(seg_logits)):
                raise RuntimeError('Encountered inf in predicted array. Aborting... If this problem persists, '
                                   'reduce value_scaling_factor in compute_gaussian or increase the dtype of '
                                   'predicted_logits to fp32')
        except Exception as e:
            del seg_logits, cls_logits, n_predictions, seg_pred, cls_pred, gaussian, workon
            empty_cache(self.device)
        ...
        with torch.no_grad():
            ...
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        seg_logits, cls_logits = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                               self.perform_everything_on_device)
                    except RuntimeError:
                        print(
                            'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                        empty_cache(self.device)
                        seg_logits, cls_logits = self._internal_predict_sliding_window_return_logits(data, slicers, False)
                else:
                    seg_logits, cls_logits = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                           self.perform_everything_on_device)
                ...
                empty_cache(self.device)
                # revert padding
                seg_logits = seg_logits[(slice(None), *slicer_revert_padding[1:])]
        return seg_logits, cls_logits
