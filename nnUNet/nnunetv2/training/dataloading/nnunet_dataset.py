...
class nnUNetDatasetMultiTask(nnUNetDataset):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
                 num_images_properties_loading_threshold: int = 0,
                 folder_with_segs_from_previous_stage: str = None):
        super().__init__(folder=folder, case_identifiers=case_identifiers,
        ...
        self.label_dict = {}
        import csv
        with open("/data/ngcminhle/nnUNet-UHN-quiz/dataset/nnUnet_raw/Dataset001_Pancreas/train_label.csv", 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                self.label_dict[row['image']] = int(row['subtype'])
        
        for c in self.dataset.keys():
            file_basename = f"{c}_0000.nii.gz"
            if file_basename in self.label_dict:
                self.dataset[c]['class_label'] = self.label_dict[file_basename]
            else:
                self.dataset[c]['class_label'] = None

        # print(self.dataset)
        # import pdb; pdb.set_trace()
        
    def load_case(self, key):
        data, seg, props = super().load_case(key)
        class_label = self.dataset[key]['class_label']
        return data, seg, props, class_label
